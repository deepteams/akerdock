...
#
# Other managed servers (ADR-078): they rebuild this exact commit from the
# public repository, natively for their own CPU, whenever their daemon lacks
# the image — validation and the agent reconciliation trigger it, so updates
# propagate with no per-server step. That requires the commit to be PUSHED;
# for uncommitted or unpushed work, ship the working tree by hand instead:
#
#   ./install.sh seed user@server
#
...
# The exact commit this build is made from (ADR-078): baked into the binary so
# every managed server rebuilds the SAME image from the public repository,
# natively for its own CPU, whenever its daemon lacks the tag — updates then
# propagate by themselves at the next validation or agent reconciliation.
# Deliberately EMPTY on a dirty tree or a tarball: a build that matches no
# commit must not claim one, and the seed subcommand below is the honest way
# to ship uncommitted work.
COMMIT=$(git rev-parse HEAD 2>/dev/null || true)
case "$VERSION" in *-dirty | src-*) COMMIT="" ;; esac

## `seed` — put this checkout's agent image onto another managed server
## (ADR-078). The image exists only in this machine's daemon, and only for
## this machine's CPU; the Dockerfile cross-compiles (its toolchains run on
## $BUILDPLATFORM), so the target's variant is built HERE without emulation
## and streamed into the server's daemon over SSH. `--output type=docker,dest=-`
## deliberately bypasses the local image store: tagging a foreign-architecture
## build as akerdock:<tag> locally would clobber the tag this very stack runs.
if [ "${1:-}" = "seed" ]; then
  shift
  [ $# -ge 1 ] || die "usage: ./install.sh seed <user@server> [extra ssh options]"
  DEST=$1; shift
  docker buildx version >/dev/null 2>&1 || die "docker buildx is required to cross-build the agent image (it ships with Docker Engine >= 24)"

  say "inspecting ${DEST}"
  remote_arch=$(ssh "$@" "$DEST" uname -m) || die "cannot reach ${DEST} over SSH"
  case "$remote_arch" in
    x86_64) PLATFORM=linux/amd64 ;;
    aarch64|arm64) PLATFORM=linux/arm64 ;;
    *) die "unsupported architecture on ${DEST}: ${remote_arch} (amd64 and arm64 only, PRD §22.4)" ;;
  esac
  # The same ladder the platform itself uses (ADR-076): bare docker for a
  # user in the docker group, passwordless sudo otherwise.
  if ssh "$@" "$DEST" docker info >/dev/null 2>&1; then
    RDOCKER=docker
  elif ssh "$@" "$DEST" sudo -n docker info >/dev/null 2>&1; then
    RDOCKER="sudo -n docker"
  else
    die "cannot talk to Docker on ${DEST} — put the SSH user in the docker group, or allow passwordless sudo (ADR-076)"
  fi
  # A clean tag names its bytes: skip the transfer when the server already
  # has it. A -dirty or timestamp tag names nothing — always rebuild and ship.
  case "$VERSION" in
    *-dirty | src-*) ;;
    *)
      if ssh "$@" "$DEST" "$RDOCKER image inspect akerdock:${VERSION}" >/dev/null 2>&1; then
        say "akerdock:${VERSION} is already on ${DEST} (${PLATFORM}) — nothing to do"
        exit 0
      fi
      ;;
  esac

  say "building akerdock:${VERSION} for ${PLATFORM} and streaming it to ${DEST}"
  note "cross-compiled from this checkout — no registry, no emulation, the local tag is untouched"
  docker buildx build --platform "$PLATFORM" \
    --build-arg VERSION="$VERSION" --build-arg IMAGE="akerdock:${VERSION}" \
    --build-arg COMMIT="$COMMIT" \
    -t "akerdock:${VERSION}" --output type=docker,dest=- . \
    | gzip | ssh "$@" "$DEST" "$RDOCKER load" \
    || die "the seed did not land on ${DEST} — read the output above"
  say "akerdock:${VERSION} is on ${DEST} (${PLATFORM})"
  note "validate (or re-validate) the server in the dashboard: the agent deploys from this tag."
  note "pushed commits propagate by themselves (ADR-078) — seed is only for work git does not hold."
  exit 0
fi
[ $# -eq 0 ] || die "unknown argument '$1' — this script takes none, or the 'seed' subcommand"
...
# AKERDOCK_TAG and AKERDOCK_COMMIT are maintained by install.sh; edit
# everything else freely.
AKERDOCK_TAG=${VERSION}
AKERDOCK_COMMIT=${COMMIT}
...
EOF
  ...
  if grep -q '^AKERDOCK_COMMIT=' .env; then
    sed -i.bak "s|^AKERDOCK_COMMIT=.*|AKERDOCK_COMMIT=${COMMIT}|" .env && rm -f .env.bak
  else
    printf 'AKERDOCK_COMMIT=%s\n' "$COMMIT" >> .env
  fi
  ...
  # AKERDOCK_TRUSTED_PROXIES is NOT seeded here: docker-compose.yml carries the
  # `gateway` default, and .env would then be a second place saying the same
  # thing — one that silently wins, and that a duplicated line turns into a
  # stale literal address nobody remembers writing.
  ...
  akerdock:
    ...
    build:
      ...
      args:
        ...
        IMAGE: akerdock:\${AKERDOCK_TAG}
        # The build's exact commit (ADR-078): managed servers rebuild this
        # image from the public repository at this commit, for their own CPU.
        # Empty (dirty tree) disables propagation rather than lying about it.
        COMMIT: \${AKERDOCK_COMMIT}
    environment:
...
## Propagation to other managed servers (ADR-078): they rebuild this exact
## commit from the public repository — which therefore has to hold it.
if [ -z "$COMMIT" ]; then
  warn "working tree is dirty (or not a git clone): this build carries no commit, so it"
  warn "cannot propagate to other managed servers (ADR-078). Commit and push, or ship"
  warn "the working tree per server:  ./install.sh seed user@server"
elif ! git branch -r --contains "$COMMIT" 2>/dev/null | grep -q .; then
  warn "commit ${COMMIT} is on no remote (as of the last fetch): managed servers build"
  warn "from the public repository and will not find it — push it, or seed (ADR-078)."
fi
...
note "other servers:     they rebuild this exact commit from the public repository,"
note "                   for their own CPU, at validation and at each update — nothing"
note "                   to run per server. Unpushed or dirty work only:"
note "                   ./install.sh seed user@server"

  ...
  template: `
    ...
    @if (loading()) {
      ...
      <div class="akd-empty">
        ...
        <p class="akd-muted">
          Enable previews in Settings, use a GitHub App source, and every pull request
          gets its own protected instance — destroyed on merge, close or TTL.
        </p>
      </div>
    } @else {
      <table class="akd-table">
        <caption class="sr-only">PR previews of this application</caption>
        <thead>
          <tr>
            <th scope="col">PR</th>
        ...
        <tbody>
          @for (p of previews(); track p.uuid) {
            <tr>
              ...
              <td>
                @if (p.fqdn) {
                  <a [href]="'https://' + p.fqdn" target="_blank" rel="noopener">{{ p.fqdn }}</a>
                } @else {

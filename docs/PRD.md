...
| Omni serving (vLLM-Omni / SGLang-Omni) as a modality of the engine | §3, §6, §26 | P3 | Compliant | ADR-083; orthogonal `modality` column (`text`|`omni`, migration 00106, immutable like engine/server/port), the engine still resolving every flag spelling and the **invocation resolved from the pair** in one runtime table (vLLM text: flags alone; vLLM omni: flags alone + the `--omni` marker on the `vllm serve` ENTRYPOINT; SGLang text: `python3 -m sglang.launch_server`; SGLang omni: `sgl-omni serve`, a separate package). `--omni` reserved in tier 2 and consumed into the modality on import, export→import identity pinned over the four runtimes; an omni model REQUIRES an explicit image (neither project publishes one to pin) refused at creation by field name; the health path moves into the runtime table and ADR-080 §4's `/v1/models` readiness clause is retired (never implemented, wrong for a model serving `/v1/audio/speech`). Multi-stage topologies deliberately out of scope: a model resource is one container. Unit tests per ADR-083 §Verification |

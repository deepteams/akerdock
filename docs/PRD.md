...
| Resource access protection (auth wall) with narrow public routes | §20.4.4, §21 | P2 | Compliant | ADR-042/049; `none`/`basic_auth`/`sso` on applications and inline Compose services; exact, `:name` template and segment-bounded prefix exceptions with explicit methods; per-service `x-akerdock.access_public_routes`; proxy-IR protected/public split, app-scoped cookie + authorize/callback ritual, API + UI, unit tests |

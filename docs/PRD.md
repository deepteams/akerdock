...
| Server agent: outbound observation push (waker merged) | §18.1, §18.3, §21.2 | P2 | Partial | ADR-040; delivered: agent_tokens + SSH-injected enrollment, push loop (docker events, wakes, heartbeat; bounded, at-least-once), `/agent/v1/observations` ingestion (server-scoped), helper ensured on every server (maintenance pass), threat-model §3.4bis, application slept/woken events + live UI, unit tests. Remaining: agent-silent surfacing in the UI |

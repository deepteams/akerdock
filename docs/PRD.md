...
| Dev ingress tunnels: a declared public URL relayed to a developer's machine | §4, §12, §21, §23 | P3 | To do | ADR-060 (Proposed); `ingress_endpoints` (stable FQDN in `domains`, ingress server, pre-provisioned router + certificate at declaration, agent offline page), mint at the control plane / redeem at the agent (`akdi_`, `akerdock-ingress-v1`), byte-splice relay entirely on the ingress server (INV-007 holds), SSO wall by default with admin-level opt-out, unconditional noindex + force-HTTPS, exclusive occupancy, idle 30 min / ceiling 12 h, CLI re-dial on transport failure only; narrow revision of ADR-031's transport invariant; unit tests per ADR-060 §Verification |

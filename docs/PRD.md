...
| CLI bastion: port-forwards to declared external endpoints | §5.7, §12, §23.3 | P3 | To do | ADR-045; `external_endpoints` resource (exact host:port, egress server, optional project/environment scope), admin-level declaration permission separated from `port-forwards:open`, mint without a client-supplied address, unchanged `akerdock-tunnel-v1` wire, two per-endpoint profiles — `standard` (today's tunnel) and `sensitive` (default: dashboard access request with a mandatory reason, fresh second factor per rbac §5, 4 h window (single grant capped at 8 h) renewable without a total, each renewal costing a fresh reason + fresh factor and extending live sessions, the grant's expiry being the session deadline (ADR-032's 4 h ceiling kept for `standard` only), revocation tearing down sessions, team notification), ADR-032's tunnel idle timeout raised 15 → 30 min for every tunnel (terminal unchanged), `authorized_until` announced at open and every automatic close reported to the CLI with an actionable reason (`grant_expired` added to `terminal_end_reason`), unit tests (validation, scope authorization, target XOR, shared team cap, teardown on deletion, both profiles, grant window, step-up factor selection, audit) |
| Scoped RBAC: role assignments per project and environment | §15, §16.3, §23.1, §27.7 | P2 | To do | ADR-046; implements ADR-007's scoped half, which ADR-038 left at the team level: base role on `team_memberships` + exception rows in `role_assignments` (project XOR environment, FK-cascaded), a fourth system role `none` without which nothing partitions, most-specific-scope-wins as an **override** that may reduce rights (union at equal scope only), the catalogue partitioned into scoped / team-read (infrastructure reads hoisted team-wide, prerequisites of deploying) / team-only (administration, `audit:read`, `secrets:reveal`, `terminal:root`), out-of-scope resources answered `404` **and filtered out of every collection and SSE stream**, token permissions intersected at the targeted resource's scope, assignment reserved to `members:manage` with anti-elevation and last-admin guard, scope named `project:<uuid>/environment:<uuid>` in the API and the audit trail while stored relationally; four slices — computed **access view first** (Access tab on applications/databases/services/projects/environments and per member, humans + API tokens with the scope that granted them, no stored attestation), then inert foundation, API+UI, convergence incl. ADR-045's `endpointInScope`; unit tests per rbac-matrix §6.2 plus the reverse-resolution agreement test and the inertia test; scoped check placed in the ~30 `resolve*` helpers (a handler without a resolver cannot compile), `projects:create` split off as team-only (creation has no parent scope), notification rules filtered while channels stay visible, invitations and SCIM defaulting to `none` in the last slice |

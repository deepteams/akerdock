...
| `AKERDOCK_TRUSTED_PROXIES` | no | — for the binary; **`gateway`** under the compose distribution, which sets it (§4.1) | Peers whose `X-Forwarded-For` / `X-Real-IP` may be believed: comma-separated IPs and CIDRs (`172.18.0.1`, `10.0.0.0/8`), or one of two shorthands. **`gateway`** — resolved at each start from the routing table — is the address this process reaches the outside world through, which is also the address the instance's own proxy appears as once its connection is NATed (`00-control-plane`, proxy-contract §5.7); it is the compose default because that address does not exist yet at install time and moves the day a custom subnet is set in the override (§4.2) — so it is resolved at each start rather than written down anywhere. Setting the variable to the **empty string** in `.env` trusts nobody again. **`private`** is RFC 1918 + loopback + link-local + ULA, for a proxy elsewhere on a private network. **Required as soon as a reverse proxy fronts the instance**, otherwise every caller address recorded is the proxy's: the audit trail attributes everything to one address, the `/auth` rate limiter gives the whole internet one shared bucket, a token's CIDR allowlist admits everyone the proxy admits, and tunnel/terminal sessions record the wrong client. Empty reads no such header at all — the right posture for a process exposed directly, since the header is written by whoever speaks last. Unparsable entry = fatal error; a `gateway` that resolves to nothing (host networking) = startup warning, never a refusal to boot. | no |
...
services:
  akerdock:
    ...
    environment:
      ...
      # caller address is the proxy's (§2.1). A variable in `.env` alone would
      # NOT reach the process — `.env` interpolates this file, it is not the
      # container's environment. `-` and not `:-`: an explicitly empty value
      # in `.env` means "trust nobody" and must survive.

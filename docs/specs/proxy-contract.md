...
| Ingress attach router/service | `<endpoint_uuid>-ingress-attach-<n>` — `n` = index of the distinct FQDN (ADR-060 §2, ADR-063) |
...
**Backend protocol (ADR-063).** Service URLs carry `http://` everywhere except one: the
reserved ingress attach service, which carries **`h2c://<agent>:<port>`** so the laptop's
control and data requests (ADR-061) stay multiplexed and natively full-duplex all the way
to the agent instead of being downgraded to one HTTP/1.1 connection per relayed stream.
The visitor routers of that same endpoint keep `http://`, and this is not tunable:
`Connection` and `Upgrade` are invalid HTTP/2 request headers (RFC 7540 §8.1.2.2) and
Traefik performs no RFC 8441 extended-`CONNECT` translation, so a relayed WebSocket
upgrade cannot survive an h2c backend hop. The agent serves both protocols on its single
internal port; its HTTP server must never set `ReadTimeout` (Go arms it as a per-stream
deadline on the HTTP/2 request body — the §5.2 trap, one hop later) and advertises more
concurrent streams than the tunnel's active + queued admission bound.
...
1. Incoming request → `docker start` (idempotent); state `waking`. The wake set is **every container of the resource** (ADR-037 §5) — for a compose stack, the whole stack minus one-shot jobs, woken **along the `depends_on` graph** (shipped container-resolved in `routes.json`), like `docker compose up`: each container starts as soon as **its own** dependencies are satisfied, so independent services wake in parallel and are never hostage to an unrelated sibling's healthcheck. A stopped dependency loses its Docker DNS alias, so waking only the routed service would boot it against a name that no longer resolves. A wake-started dependency must be **ready** (healthy, or running-stable) before its dependents start; edges to one-shot jobs are satisfied by construction (they ran at deploy).
2. Wait for the healthcheck (`healthy`, or `running` stable for 10 s without healthcheck), timeout **60 s (proposed default)**. Only containers **this wake started** are gated on readiness: a container already running when the wake begins (e.g. running but unhealthy) is never awaited — its health surfaces through the proxied response, exactly as without scale-to-zero. Gating it would hold every request of the resource on the single-flight lock, one wake budget per request, indefinitely.
...
4. **Replay**: the held request is forwarded to the container (hold-and-forward); subsequent requests take the normal path as soon as Traefik picks up the file. For a browser navigation (`GET`/`HEAD` with `Accept: text/html`) the waker serves an **auto-refreshing waiting page** instead of the hold: `503` + `Retry-After: 2` + `X-AkerDock-Scale: waking`, self-contained HTML listing each container of the wake set with its live state (waiting / starting / running / ready / unhealthy), while the wake runs in the background (single-flight). The page stays up **until the background wake completes**, even once every container is running — the graph wake starts everything within seconds, and handing off earlier would hide the starting→ready progression (and queue the navigation on the wake gate). A failed wake renders its cause, stops refreshing and offers a retry link (`?akd_wake_retry=1`, stripped before proxying); other methods keep hold-and-forward — a form body cannot survive a waiting page.
...
- **Wake timeout**: 60 s (proposed default) **per progress step** — the deadline re-arms each time a container is newly released, so a five-service stack is not asked to cold-start inside the budget of a single container (the deploy budgets per service the same way), while a genuinely stuck container still fails within one budget. On overrun: `504` naming the blocking container (or the failure variant of the waiting page) and the waker **stops the containers this wake started** (rollback, reverse order) so the resource returns to `sleeping` — never left half-awake in a restart crash-loop the control plane believes asleep.

...
- **Domaines au niveau application** (champ Routing de l'UI) : sur une stack compose, ils sont résolus vers le **service web** du stack, déterministiquement — service dont le premier `expose` correspond au `target_port` du domaine (`fqdn:port`), sinon l'**unique** service exposant un port. Aucun service exposé → `compose_routable_port_unresolved` ; plusieurs candidats sans `target_port` discriminant → `compose_routable_component_ambiguous`. Jamais de conteneur deviné : un domaine d'application ne cible jamais le nom du stack (ce conteneur n'existe pas en compose).

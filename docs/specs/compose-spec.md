...
| Public endpoints through the stack access wall | `x-akerdock.access_public_routes` | `services.<name>` | ADR-049. Narrow unauthenticated exceptions for domains routed to this service only. Each entry requires an absolute `path` and explicit `methods`; `match` is `exact` (default), `template` (`:name` = one unreserved URL segment), or segment-bounded `prefix`. `parameters` optionally restricts template values. Regex and globs are rejected. |
...
| `compose_access_public_route_invalid` | error | Invalid `x-akerdock.access_public_routes` entry: unsafe path/pattern, missing methods, or invalid template parameter (§5.1, ADR-049) |
